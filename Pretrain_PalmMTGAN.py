...
SAVE_PATH = "/content/drive/MyDrive/S3/GAN/Eksperimen_PalmMTGAN_V3"
LOG_PATH = os.path.join(SAVE_PATH, "pretrain_v3_log.txt")
CSV_PATH = os.path.join(SAVE_PATH, "pretrain_v3_metrics.csv")
VISUAL_PATH = os.path.join(SAVE_PATH, "Visuals_pretrain_v3")
...
LR = 1e-4
GRAD_CLIP = 1.0
SEG_WEIGHT = 3.0            # Focal‑Dice loss, bobot tetap 3.0
PATIENCE = 10
...
def main():
    write_log("🚀 PRETRAINING V3 (SPM + Focal‑Dice Loss)")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for lr, hr, mask, name in pbar:
            ...
            with torch.amp.autocast('cuda'):
                sr, seg_logits, feat_sr, feat_seg = netG(lr)
        ...
        for tag, (iou_val, lr_img, hr_img, sr_img, mask_img, seg_img, name_img) in [
                ("worst", worst), ("best", best)]:
            save_visuals_grid_header(
                lr_img.to(DEVICE), hr_img.to(DEVICE), sr_img.to(DEVICE),
        ...
        if avg_iou > best_iou:
            ...
            torch.save(netG.state_dict(), os.path.join(SAVE_PATH, 'best_pretrained_IoU_v3.pth'))
            write_log(f"⭐ Rekor IoU baru: {best_iou:.4f} → model disimpan")
            early_stop_counter = 0
        else:
    ...
    csv_file.close()
    write_log(f"✅ PRETRAINING V3 SELESAI. Best IoU: {best_iou:.4f} di epoch {best_iou_epoch}")

if __name__ == "__main__":
    try:
        main()
        print("\n[SUCCESS] Pretraining selesai.")
    except Exception as e:
        print(f"\n[ERROR] Pretraining terhenti karena: {e}")
    finally:
        try:
